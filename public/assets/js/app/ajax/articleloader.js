...
//p(category_segment);
	...
	$(window).scroll(function () {
		...
		//記事読み込みajax発火
		//--------------------
		if(($('html').height() - scrollBottom)  <= 400) {
			// フラグが立っていたら本発火
			if(fireflag == true) {
				// 記事のナンバーを取得
				article_number = $('.main_contents').children('.article_list:last-child').attr('data-article_number');
//				p(article_number);
				// フラグを折る
				fireflag = false;
				// ローディング表示
				...
				$.ajax({
					type: 'GET', 
					url: http + 'app/ajax/articleloader/',
					data: {
...
//						p(data);
						...
						$('.ajax_loader').remove();
						// エラー文挿入
						ajax_loader_html = '<div class="ajax_error">ごめんなさい！読み込みエラーです。</div>';
						// 表示
						$('.main_contents').append(ajax_loader_html);
				  },

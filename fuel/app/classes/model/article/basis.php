...
//$user_data_array["REMOTE_HOST"] = 'sp49-104-4-249.msf.spmode.ne.jp';
	...
	static function cron_article_access_writing_and_all_page_view_plus($method, $user_data_array, $sharetube_id) {
//		pre_var_dump($method);
//		pre_var_dump($user_data_array);
		$is_access = true;
		// アクセス禁止名
...
//$user_data_array["REMOTE_HOST"] = 'sp49-104-4-249.msf.spmode.ne.jp';
		...
		if($is_access) {
			...
			$access_res = mysql_query("
				...
				ORDER BY primary_id DESC 
				LIMIT 0, 1");
			while($row = mysql_fetch_assoc($access_res)) {
				$REMOTE_HOST = $row['REMOTE_HOST'];
			}
// ローカルテスト
//$user_data_array["REMOTE_HOST"] = rand();
			// f5攻撃を防ぐ
			if($REMOTE_HOST != $user_data_array["REMOTE_HOST"]) {
				// アクセス書き込み
			 mysql_query("
				...
				while($row = mysql_fetch_assoc($user_res)) {
					...
					$pay_pv        = (int)$row["pay_pv"];
					$all_page_view = (int)$row["all_page_view"];
					$pay_pv++;
					$all_page_view++;
					// 更新
					mysql_query("
...
//				Model_Article_Basis::access_summary_writing($method, $article_res, $user_data_array);
		...
		$access_summary_res = mysql_query("
			SELECT *
			FROM access_summary
			...
			AND hour  = ".$access_article_hour_time."");
		while($row = mysql_fetch_assoc($access_summary_res)) {
			$access_summary_array["primary_id"] = (int)$row["primary_id"];
			$access_summary_array["article_id"] = (int)$row["article_id"];
			$access_summary_array["count"]      = (int)$row["count"];
				...
				// あったら加算してアップデート
					...
					mysql_query("
						UPDATE access_summary
							...
							WHERE primary_id = ".$access_summary_array["primary_id"]."");
				}
			} // if(!$REMOTE_HOST  == $user_data_array["REMOTE_HOST"]) {
		} // if($is_access) {
	}
...
//				var_dump($article_access_res);
	...
	//殿堂まとめのリスト取得
	//----------------------
	public static function fame_article_list_get($get_num = 10, $page_num = 1) {
		// 取得する場所取得
		$start_list_num = ($page_num*$get_num)-$get_num;
		$fame_article_array = array();
		...
		foreach($fame_article_res as $key => $value) {
			$article_res = DB::query("
				SELECT primary_id, sharetube_id, category, title, sub_text, tag, thumbnail_image, sp_thumbnail, link, matome_frg, create_time, update_time
				FROM article
				WHERE primary_id = ".$value['article_id']."
				AND del = 0")->cached(86400)->execute();
			foreach($article_res as $article_key => $article_value) {
				$fame_article_array[$key] = $article_value;
			}
		} // foreach($fame_article_res as $key => $value) {
		return $fame_article_array;
	}
	//------------------------------
	//殿堂まとめページングデータ取得
	//------------------------------
	public static function fame_article_paging_data_get($list_num, $paging_num) {
		// last_num取得
		$max_res = DB::query("
			SELECT COUNT(primary_id)
			FROM fame_article
		...
		// 最大ページング数取得
		$max_paging_num = (int)ceil($last_num/$list_num);
		// fame_article_paging_data生成
		$fame_article_paging_data_array = array(
			'last_num'       => $last_num,
			'list_num'       => $list_num,
			'paging_num'     => $paging_num,
			'max_paging_num' => $max_paging_num,
		);
		return $fame_article_paging_data_array;
	}
